...
def centerBreak(text, width):
	'''
	Return a string centered-aligned obeying newline breaks within the given width
	'''
	lines = text.split('\n')
	centeredBreakLines = map(lambda line: center(line, width), lines)
	return '\n'.join(centeredBreakLines)
